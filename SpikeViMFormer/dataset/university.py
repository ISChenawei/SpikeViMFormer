...
class RandomKeepPatches(ImageOnlyTransform):
    def __init__(self, keep_ratio=0.1, num_patches=5, always_apply=False, p=1.0):
        """
        :param keep_ratio: 保留的图像面积比例 (0-1)，比如 0.1 = 保留 10% 区域
        :param num_patches: 保留区域的块数
        """
        super(RandomKeepPatches, self).__init__(always_apply, p)
    ...
    def apply(self, img, **params):
        h, w, c = img.shape
        total_area = h * w
        keep_area = int(total_area * self.keep_ratio)
        area_per_patch = keep_area // self.num_patches
        ...
        for _ in range(self.num_patches):
            ...
            patch_h = int(np.sqrt(area_per_patch / aspect_ratio))
            patch_w = int(area_per_patch / max(patch_h, 1))
...
def get_transforms(img_size,
    ...
    val_transforms = A.Compose([A.Resize(img_size[0], img_size[1], interpolation=cv2.INTER_LINEAR_EXACT, p=1.0),
                                # RandomBlackPatches(area_ratio=0.1, always_apply=False, p=0.8),
                                # Multi-weather U1652 settings.
                                # iaa_weather_list[weather_id],
                                # RandomKeepPatches(keep_ratio=1,num_patches=10,p=1),
    ...
    train_sat_transforms = A.Compose([A.ImageCompression(quality_lower=90, quality_upper=100, p=0.5),
                                      ...
                                      # A.RandomCrop(width=int(0.9*img_size[0]),height=int(0.9*img_size[1]),p = 0.5),
                                      A.Normalize(mean, std),
                                      ToTensorV2(),
                                      ])
